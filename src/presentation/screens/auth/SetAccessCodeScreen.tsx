...
export default function SetAccessCodeScreen() {
  ...
  const save = useCallback(async (value: string) => {
    ...
    setError('');
    try {
      await setAccessCode(value, label.trim() || undefined);
      showSuccess(
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      <AppHeader
        title={route.params?.firstTime ? 'Crea tu clave' : 'Clave de acceso'}
        showBack={!mandatory}
      ...
      <ScrollView
        ...
        <CustomInputComponent
          nameInput={isConfirm ? 'Repite la clave' : 'Clave de acceso'}
          placeholder="Ej. K7M2Q4"
          value={isConfirm ? confirmCode : code}
          onChangeText={v => {
            const next = v.toUpperCase();
            if (isConfirm) setConfirmCode(next); else setCode(next);
            if (error) setError('');
          }}
        ...
        <CustomButtonComponent
          ...
          onPress={() =>
            isConfirm ? handleConfirmComplete(confirmCode) : handleCreateComplete(code)
          }
          isLoading={isSaving}
          disabled={(isConfirm ? confirmCode : code).length !== ACCESS_CODE_LENGTH}
          loaderColor="#FFFFFF"
          style={{
            backgroundColor:
              (isConfirm ? confirmCode : code).length === ACCESS_CODE_LENGTH
                ? colors.primary
                : colors.border,
          }}
